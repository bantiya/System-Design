...
These notes combine four revision areas:

1. **Delivery Framework** — how to structure the interview, manage time, and deliver a working design.
2. **Core Concepts** — technology-agnostic fundamentals that show up in almost every system design interview.
3. **Key Technologies** — the building blocks you can use in most system design interviews.
4. **Common Patterns** — repeatable architectural patterns that help you solve common interview problems quickly.
...
The delivery framework helps you avoid that problem by giving you a reliable sequence:
...
The goal of the requirements section is to clearly understand what system you are designing. This is where you narrow a broad prompt into a focused problem.
...
You can optionally discuss capacity estimates, but only if the numbers will affect your design.

---

## 4. Functional Requirements

Functional requirements describe what users or clients should be able to do.
...
These are the core product features.
...
Keep the list focused. Most real products have dozens or hundreds of features, but you should usually prioritize the top 3 core requirements.

Too many requirements can hurt your answer because every requirement must be supported in the architecture.
...
Non-functional requirements describe system qualities. They are about **how well** the system should work.
...
|---|---|
| CAP / Consistency | Should the system prioritize consistency or availability during failures? |
...
Many candidates start system design interviews by calculating DAU, QPS, storage, bandwidth, and number of servers. That is not always necessary.
...
For example, capacity estimation is useful when deciding:
...
This is usually better than calculating a lot of numbers and then only concluding:

```text
The system is large.
```

That does not help the design.
...
After requirements, identify the core entities. These are the main nouns in the system. They are usually the objects your APIs exchange and your database persists.
...
At this point, do not list every column or field. You do not yet know all the access patterns and state transitions.
...
These fields matter because they help prevent double booking.
...
## 8. Step 3 — API or System Interface (~5 Minutes)

Before drawing the architecture, define how clients interact with the system. The API acts as the contract between the outside world and your backend.

For most product-style interviews, default to REST.

```text
Client → REST API → Backend System
```

Use GraphQL only if clients need flexible data fetching. Use gRPC mainly for internal service-to-service communication when performance matters.
...
Do not trust sensitive user identity from request body.
...
For some systems, especially backend processing systems, it helps to describe the data flow before drawing components.

Use this when the system has a sequence of processing stages.
...
Skip this section if the system is mostly simple CRUD or request-response. You do not need a separate data flow for every design.

---

## 12. Step 4 — High-Level Design (~10–15 Minutes)

The high-level design is where you draw the main architecture using boxes and arrows.

Your goal is to produce a working system that satisfies the APIs and requirements.
...
A reliable way to build the architecture is to walk through each important API.
...
This shows you recognize the bottleneck without derailing the high-level design too early.

---

## 14. What to Include in the High-Level Design

Include the components that are necessary for the core requirements.
...
You do not need full schemas, but add fields that matter to the design.
...
These fields explain how the system handles seat state and reservation expiration.
...
The deep dive section is where you harden the design. You take the basic working system and improve it based on the non-functional requirements.

Deep dives usually cover:
...
For senior candidates, this is especially important. Senior-level performance often comes from proactively identifying the most interesting bottlenecks rather than waiting for the interviewer to point them out.
...
```text
NFR: Match riders with nearby drivers at low latency.
...
The delivery framework is about producing a complete, understandable design under time pressure.
...
The strongest candidates do not just list technologies. They manage the interview, make reasonable trade-offs, and guide the interviewer through a design that starts simple and becomes more robust.
...
Core concepts are the foundation of system design. Technologies such as Redis, Kafka, Postgres, S3, and Elasticsearch are tools, but core concepts explain **why** and **when** those tools are useful.

Think of core concepts as the vocabulary of system design. Before designing Instagram, Uber, Ticketmaster, YouTube, or Dropbox, you need to understand ideas like:
...
A strong interviewer is not only checking whether you know a tool name. They are checking whether you understand the underlying principle and the trade-offs.
...
A good system design answer usually starts simple, then brings in these concepts only when the requirements demand them.
...
Do not force every concept into every answer. Use the ones that are relevant to the bottleneck or failure mode you are solving.
...
Networking is about how clients, services, databases, queues, caches, and other components communicate with each other. In system design interviews, you do not need to go deep into every OSI layer, but you should understand the practical choices that affect latency, reliability, and scalability.

Most systems use **HTTP over TCP** as the default communication model. It is reliable, widely supported, easy to debug, and works well for normal request-response APIs.
...
Realtime communication → SSE or WebSockets if needed
```
...
HTTP is usually enough for normal CRUD APIs, search APIs, booking APIs, and user-facing application flows.
...
gRPC is often useful for internal service-to-service communication when performance matters. It uses HTTP/2 and binary serialization, which can be faster and more compact than JSON over HTTP.
...
|---|---|---|---|
| L4 | TCP/UDP | Connection information | WebSockets, high throughput, persistent connections |
| L7 | HTTP/application | URL, headers, cookies, request content | REST APIs, path routing, service routing |
...
The goal is to show that you understand the product flow and the main system operations.
...
In interviews, do not spend too long designing APIs. Usually, 4–6 core endpoints are enough.
...
Cursor-based pagination is usually better for feeds and realtime lists because new items can be inserted while the user is paging.
...
```text
User authentication → JWT or session token
Service-to-service authentication → API key, mTLS, service identity
```

For system design interviews, it is usually enough to mention that requests go through authentication and authorization at the API gateway or service layer.
...
This makes search fast, but the search index may lag behind the primary database. For search use cases, slight staleness is usually acceptable.
...
Caching stores frequently accessed data in faster storage, usually memory, so the system can avoid repeated expensive database queries or computations.
...
A Redis cache lookup is usually much faster than a database query, so caching can significantly improve read-heavy systems.
...
In real distributed systems, network partitions can happen. So during a partition, the system usually chooses between consistency and availability.
...
If a like count is stale for a few seconds, the business impact is usually low.
...
You do not need one consistency model for the entire system.
...
Back-of-the-envelope estimates help you decide when to scale. You do not need to start every interview with calculations, but you should use numbers when making architecture decisions.
...
| SSD read | Microseconds to low milliseconds |
...
| App server | Can handle many concurrent requests depending on CPU, memory, and workload | CPU > 70%, latency over SLA, memory pressure |
...
```text
10 million users
Each user stores 1 MB of metadata
Total metadata = 10 TB
```
...
System design is mostly about assembling the right building blocks to solve a specific problem. You do not need to know every vendor or every managed service, but you should know at least one strong option in each major technology category.

For example, interviewers usually do not care whether you choose **Kafka**, **SQS**, or another queueing system, as long as you understand what a queue is, when it helps, and what trade-offs it introduces. But if you do not know any queueing technology at all, it becomes difficult to design systems with background processing, burst handling, or asynchronous workflows.
...
The goal is not to name-drop technologies. The goal is to show that you know what each technology does, when to use it, and what problems it creates.
...
| Relational Database | Store transactional data with SQL, joins, indexes, and ACID transactions |
| NoSQL Database | Store flexible or high-scale data using key-value, document, column-family, or graph models |
...
| CDN | Cache content close to users globally |
...
Do not spend too much time giving a generic SQL vs NoSQL comparison unless the interviewer asks. A stronger answer is to choose one database and explain why it fits the specific problem.

Good example:

> I would use Postgres here because the system needs transactional consistency for orders, payments, and inventory updates. Postgres gives us ACID transactions, indexes for common queries, and strong data integrity.
...
If you do not have a favorite, PostgreSQL is a safe interview choice because it supports strong transactional behavior, rich indexing, JSON fields, full-text search, and extensions like PostGIS.
...
The app server should usually issue temporary permissions, not handle the file bytes itself.
...
Blob storage systems replicate data and are designed for very high durability. In an interview, you can usually treat managed blob storage as highly durable and scalable.
...
Do not use S3 or blob storage as your primary query database unless you have a very specific reason. It is good for large objects, not low-latency indexed queries.
...
A query like this is usually slow at scale:
...
Do not spend too much time deep-diving into the gateway unless the problem is specifically about routing, authentication, rate limiting, or API management. Usually, mention it briefly and move on to the core system behavior.
...
You do not need to draw a load balancer in front of every service. It is usually enough to draw one at the front of the system and mention that internal services are horizontally scaled behind load balancers.

### L4 vs L7 Load Balancing

| Type | Works At | Good For |
|---|---|---|
| L4 Load Balancer | TCP/UDP connection level | WebSockets, persistent connections, high throughput |
| L7 Load Balancer | HTTP/application level | Path-based routing, header-based routing, flexible API routing |

Simple rule:

```text
Persistent connections like WebSockets → usually L4
Normal HTTP APIs → usually L7
```
...
Many queues provide FIFO ordering, but ordering may only be guaranteed within a partition or message group.
...
A stream is an append-only sequence of events that consumers can read from. Unlike a simple queue, streams usually retain data for a configurable period, allowing consumers to replay events.
...
| Queue | Stream |
|---|---|
| Message is usually consumed and removed or hidden | Events are retained for replay |
...
Distributed locks should usually have a TTL so they do not stay locked forever if the process crashes.
...
| FIFO | Remove oldest inserted items first |
...
Do not just say “store it in Redis.” Explain the structure.
...
A Content Delivery Network is a globally distributed cache that serves content from locations close to users.
...
Do not say “use CDN” without explaining what content is cacheable. User-specific or highly sensitive data usually should not be cached publicly unless the system uses private caching, signed URLs, or strict cache controls.
...
In a system design interview, you usually do not have enough time to invent every solution from scratch. Patterns help you quickly recognize the type of problem being asked and apply a known architectural approach.
...
The goal is not to memorize one fixed architecture. The goal is to identify the problem type, explain the trade-offs, start simple, and add complexity only when the requirements demand it.
...
For realtime updates, you usually need a way to route messages to the correct connected users.
...
A synchronous API should usually return quickly. If a user request takes 30 seconds, 2 minutes, or 10 minutes, the system may timeout or provide a poor user experience.
...
This is usually the simplest and best starting point.
...
Do not just say “add Redis.” Explain what you cache, cache key design, TTL, invalidation strategy, and what happens on cache miss.
...
A better approach usually uses a hash-based partition key:
...
Do not upload large videos through your backend server unless there is a very specific reason. Use presigned URLs and direct-to-storage uploads.
...
A strong answer mentions durable state, not just “call services one by one.”
...
A strong system design interview answer shows that you understand both the pattern and the trade-offs. The interviewer is usually not just checking whether you know Redis, Kafka, WebSockets, or S3. They are checking whether you know when each tool is actually justified.
